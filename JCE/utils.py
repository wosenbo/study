...
class Reg:
    ...
    def writeTo(self, t: tarscore.TarsOutputStream):
        t.write(tarscore.vctclass(tarscore.string), 0, self.vGroupId)
        t.write(tarscore.string, 1, self.sToken)
       

    def readFrom(self, t: tarscore.TarsInputStream):
        self.vGroupId = t.read(tarscore.bytes, 0, False, self.vGroupId)
        self.sToken = t.read(tarscore.string, 1, False, self.sToken)
...
class WSUserInfo:
    ...
    def readFrom(self, t: tarscore.TarsInputStream):
        ...
        self.lUid = t.read(tarscore.int64, 0, False,self.lUid)
        self.bAnonymous = t.read(tarscore.boolean, 1, False,self.bAnonymous)
        self.sGuid = t.read(tarscore.string, 2, False,self.sGuid)
        self.sToken = t.read(tarscore.string, 3, False,self.sToken)
        self.lTid = t.read(tarscore.int64, 4, False,self.lTid)
        self.lSid = t.read(tarscore.int64, 5, False,self.lSid)
        self.lGroupId = t.read(tarscore.int64, 6, False,self.lGroupId)
        self.lGroupType = t.read(tarscore.int64, 7, False,self.lGroupType)
...
class MessageNotice:
    ...
    def readFrom2(self, t: tarscore.TarsInputStream):
        self.msgid = t.read(tarscore.int64, 0, False, self.msgid)
        self.vvipnum = t.read(tarscore.int32, 1, False, self.vvipnum)
        
        pass

    def readFrom3(self, t: tarscore.TarsInputStream):

        self.data = t.read(tarscore.vctclass(tarscore.string),0, False, self.data)
      
        self.data2 = t.read(tarscore.string, 1, False, self.data2)
       
        self.data3 = t.read(tarscore.string, 2, False, self.data3)
